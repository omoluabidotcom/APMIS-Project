...
public class ReportView extends VerticalLayout implements RouterLayout{
	...
	private Map<Tab, Supplier<Component>> tabComponentSupplierMap = new LinkedHashMap<>();
	private Map<Tab, Component> loadedComponents = new LinkedHashMap<>();

	private Tabs createTabs() {
		System.out.println("creating tabs");

		tabComponentSupplierMap.put(new Tab(I18nProperties.getCaption(Captions.aggregateReport)), () -> new AggregateReportView());
		tabComponentSupplierMap.put(new Tab("Data Completness Reports"), () -> new CompletionAnalysisTabsheet());
		tabComponentSupplierMap.put(new Tab(I18nProperties.getCaption(Captions.mobileUsers)), () -> new UserAnalysisView());
		// tabComponentSupplierMap.put(new Tab(I18nProperties.getCaption(Captions.dataTimeliness)), () -> new DataTimelinessReportTab());

		System.out.println("creating tabs created");

		return new Tabs(tabComponentSupplierMap.keySet().toArray(new Tab[] {}));
	}
	...
	private Component getOrCreateComponent(Tab tab) {		
		...
		Supplier<Component> supplier = tabComponentSupplierMap.get(tab);
		if (supplier != null) {
