...
public class ControlTextEditFieldRange extends ControlPropertyEditField<String> {
    ...
    protected void setUpOnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                ...
                if (imm != null) {
                    if (hasFocus) {
                        ...
                        // Focus lost - trigger value change notification
    ...
    protected void setFieldValue(String value) {
        System.out.println("value from setfield value " + value);
        input.setText(value);
        return;
    }
...
//                }
            ...
            public void afterTextChanged(Editable editablex) {
                ...
                // Special handling for "0" - clear errors immediately
                if (text != null ) {
                    input.setError(null);
                    disableErrorState();
                    changeVisualState(VisualState.NORMAL);
                    System.out.println("Zero input - errors cleared");
                }
                ...
                if (isRange && minValue != null && maxValue != null) {
                    if (!text.isEmpty()) {
                        try {
                            ...
                            if (valxx >= minValue && valxx <= maxValue) {
                                // Valid range - clear any errors
                                input.setError(null);
                                disableErrorState();
                            } else if (warnOnError) {
                                NotificationHelper.showNotification((NotificationContext) input.getContext(),
                                        WARNING, "Number not in provided range! i.e min: " + minValue + " and max: " + maxValue);
                            } else {
                                // Only show error if value is not 0 (allow 0 even if out of range)
                                if (valxx != 0) {
                                    input.setError("Number not in provided range! i.e min: " + minValue + " and max: " + maxValue);
                                    enableErrorState("Number not in provided range! i.e min: " + minValue + " and max: " + maxValue);
                                } else {
                                    // Clear error for 0 value
                                    input.setError(null);
                                    disableErrorState();
                                }
                            }
                        } catch(NumberFormatException e) {
                            // Check if it's "0" - if so, clear error
                            if (text.equals("0")) {
                                input.setError(null);
                                disableErrorState();
                            } else if(warnOnError) {
                        ...
                        // For empty field, only show error if required
                        if (beforeData.length() == 0 && onChangeData.length() == 0) {
                            System.out.println("lenght before and after is 0 ===== no user input ==");
                            disableErrorState();
                        }else{
                            if (isRequired) {
                                enableErrorState("This field is required");
                            } else {
                                disableErrorState();
                            }
                        }
...
//                    System.out.println(beforeDatax.length() + "beforeXDatarange111111111111111111111111111111111" + onChangeData.length());
                    ...
                    System.out.println("else range but not expression ==================");
                    try {
                        if (!text.isEmpty()) {
                            int valxx = Integer.parseInt(text);
